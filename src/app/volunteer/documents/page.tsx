...
export default function VolunteerDocumentsPage() {
        ...
        await Promise.all(
          imgs.map(async (d) => {
            ...
            try {
              ...
              const token = session?.access_token
              const res = await fetch(`/api/volunteer-documents?id=${encodeURIComponent(d.id)}&signed=1`, {
                method: "GET",
                headers: token ? { Authorization: `Bearer ${token}` } : undefined,
                credentials: "include",
                cache: "no-store",
  ...
  const ensureSignedUrl = async (docId: string) => {
    ...
    try {
      ...
      const token = session?.access_token
      const res = await fetch(`/api/volunteer-documents?id=${encodeURIComponent(docId)}&signed=1`, {
        method: "GET",
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
        credentials: "include",
        cache: "no-store",
  ...
  const viewDoc = async (id: string) => {
    try {
      ...
      const token = session?.access_token
      const res = await fetch(`/api/volunteer-documents?id=${encodeURIComponent(id)}&signed=1`, {
        method: "GET",
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
        credentials: "include",
        cache: "no-store",
  ...
  const upload = async () => {
    ...
    try {
      ...
      const uploadJson = await new Promise<any>((resolve, reject) => {
        ...
        xhr.open("POST", "/api/volunteer-documents", true)
    ...
    <VolunteerLayout>
      <div className="p-4 md:p-6 space-y-3 md:space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <h1 className="text-xl md:text-2xl font-semibold text-gray-900 dark:text-white">My Documents</h1>
        ...
        <Card className="p-3 md:p-4 space-y-3">
          <div className="flex flex-col sm:flex-row gap-2 md:gap-3 items-stretch sm:items-center">
            ...
            <Button
              ...
              className="w-full sm:w-auto whitespace-nowrap text-xs md:text-sm h-9 md:h-10 sm:mt-auto bg-green-600 hover:bg-green-700"
            >
              {uploading ? (
                <>
        ...
        {uploading && uploadProgress !== null && (
          <Card className="p-3 md:p-4 border-green-200 dark:border-green-800">
            ...
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
              <div
                ...
                className="h-2 bg-green-600 dark:bg-green-500 transition-all duration-300"
              />
            </div>
          </Card>
        )}
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex gap-1">
                                  <Button
                                    ...
                                    className="h-6 text-[10px] md:text-xs px-2 bg-green-600 hover:bg-green-700"
                                  >
                                    Save
                                  </Button>
                                  <Button
                          ...
                          {renamingDoc !== d.id && (
                            <div className="flex flex-col gap-1 flex-shrink-0">
                              ...
                              <Button
                                ...
                                className="h-6 md:h-7 text-[10px] md:text-xs px-2 text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300 hover:bg-green-50 dark:hover:bg-green-900/30"
                              >
                                Rename
                              </Button>
                              <Button
